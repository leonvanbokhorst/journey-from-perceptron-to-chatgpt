...
from exercises.module5.code.ex_01_seq2seq_basics import Encoder as BasicEncoder
from exercises.module5.code.ex_01_seq2seq_basics import (
    one_hot_encode,
    generate_simple_translation_dataset,
)
from exercises.module5.code.ex_01_seq2seq_basics import Seq2Seq
...
class AttentionMechanism:
    ...
    def compute_attention_weights(
        self, decoder_h: np.ndarray, encoder_hs: List[np.ndarray]
    ) -> np.ndarray:
        """
        ...
        for h_enc in encoder_hs:
            ...
            score = np.dot(
                self.v.T, np.tanh(np.dot(self.W1, decoder_h) + np.dot(self.W2, h_enc))
            )
    ...
    def compute_context_vector(
        self, encoder_hs: List[np.ndarray], attention_weights: np.ndarray
    ) -> np.ndarray:
        """
...
def visualize_translation_with_attention(
    input_str: str, output_str: str, attention_matrix: np.ndarray
) -> None:
    """
...
def compare_models_on_translation() -> None:
    ...
    basic_losses = seq2seq.train(
        X_train[:200], y_train[:200], epochs=30, learning_rate=0.01, print_every=5
    )

    print("\nTraining Seq2Seq model with attention...")
    attn_losses = seq2seq_attn.train(
        X_train[:200], y_train[:200], epochs=30, learning_rate=0.01, print_every=5
    )
    ...
    for i in range(3):
        ...
        # Visualize attention
        visualize_translation_with_attention(
            input_text, attn_translation, attention_matrix
        )
...
def main() -> None:
    ...
    print(
        "1. Experiment with different attention mechanisms (e.g., dot product attention)."
    )
    ...
    print(
        "4. Compare performance when increasing sequence lengths (attention helps with longer sequences)."
    )
